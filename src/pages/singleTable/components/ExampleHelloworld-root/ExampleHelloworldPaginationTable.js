...
export default class ExampleHelloworldPaginationTable extends Component {
    ...
    componentDidMount(){
        ...
        actions.ExampleHelloworld.loadList();//table数据
    }
    ...
    cellClick = async (record,btnFlag) => {
        await actions.ExampleHelloworld.updateState({
            rowData : record,
        });
        ...
        actions.routing.push(
            {
                pathname: 'ExampleHelloworld-edit',
    ...
    // 流程提交成功后回调函数
    onSubmitSuc = async ()=>{
        await actions.ExampleHelloworld.loadList();
    ...
    onExamine = async (text, record, index)=> {
        ...
        await actions.ExampleHelloworld.updateState({rowData:record});
    ...
    onModalDel = async (delFlag)=>{
        ...
        if(delFlag){
            await actions.ExampleHelloworld.delItem({
                param: delData
            });
        }
        this.setState({
